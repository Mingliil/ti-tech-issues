...
class_name Cabo extends Node3D
...
@export_group("Conexoes")
@export var ConexaoInicio: PackedScene = null:
	set(value):
		ConexaoInicio = value
		if Engine.is_editor_hint():
			_regenerate_chain()
@export var ConexaoFim: PackedScene = null:
	set(value):
		ConexaoFim = value
		if Engine.is_editor_hint():
			_regenerate_chain()

@export_group("Dispositivos Conectados")
@export var dispositivoConectadoInicio: Node
@export var dispositivoConectadoFim: Node

@export_group("referencias")
@export var link_inicio: Node3D
@export var link_fim: Node3D
...
func _generate_chain() -> void:
	...
	for i in range(link_count):
		...
		link_inicio.add_child(link)
		links.append(link)
		link.position = Vector3(0, -(i + 1) * link_length, 0)
	await  get_tree().process_frame
	
	for i in range(link_count):
		var body_a = link_inicio if i == 0 else links [i-1]
		var body_b = links[i]
	...
	if ConexaoFim and links.size() >0:
		var conexao = ConexaoFim.instantiate()
		link_fim.add_child(conexao)
		var bottom_link = links[links.size()-1]
		conexao.global_position = bottom_link.global_position + Vector3(0, -link_length, 0)
		if conexao is RigidBody3D:
			var joint = _create_joint(bottom_link, conexao)
			conexao.add_child(joint)
	if ConexaoInicio and links.size() >0:
		var conexao = ConexaoInicio.instantiate()
		link_fim.add_child(conexao)
		var top_link = links[0]
		conexao.global_position = top_link.global_position + Vector3(0, -link_length, 0)
		conexao.global_rotation = top_link.global_rotation + Vector3(0,-link_length-180,0)
		if conexao is RigidBody3D:
			var joint = _create_joint(top_link, conexao)
			conexao.add_child(joint)
...
func _clear_chain() -> void:
	...
	links.clear()
	joints.clear()
	for child in link_fim.get_children():
		child.queue_free()
